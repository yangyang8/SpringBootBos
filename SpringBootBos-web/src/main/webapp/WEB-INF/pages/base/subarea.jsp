...
<script type="text/javascript">
	...
	function doExport(){
		//alert("导出");
		//进行向服务器发送导出请求,刷新页面
		location.href="/subarea/exportSubarea.action";
	}
	...
	$(function(){
		...
		$("#btn").click(function(){
			//alert("执行查询...");
			//进行发送ajax请求，把条件传给我们的服务器，让服务器进行查询操作
			//得到条件查询
			/* var json={
					"region.province":$("input[region.province]"),
					"region.city":$("input[region.city]"),
					"region.district":$("input[region.district]")
					}; */
			   var json={
					"region.province":$("#province").val(),
					"region.city":$("#city").val(),
					"region.district":$("#district").val(),
					"addresskey":$("#addresskey").val()
					}; 
			$('#grid').datagrid("load",json);//会重要去执行我们上面写的$('#grid').datagrid( {})方法 
		...
		$("#save").click(function(){
			//进行校验数据
			save("saveSubareaForm");//提取出来的工具类的方法
		});
		
		$("#edit").click(function(){
			save("editSubareaForm");
		});
	...
	function doDblClickRow(rowIndex, rowData){
		...
		//alert(rowData.region.province);
		//手动设置checkBox当中的值$('#cc').combobox('setValue', '001'); 
		$("#myregion").combobox("select",rowData.region.province+" "+rowData.region.city+" "+rowData.region.district);
		$('#myregion').combobox('setValue',rowData.region.id); 
		//$("#myregion").combobox("select",rowData.name);
	}
</script>	
</head>
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="分区添加修改" id="addSubareaWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="saveSubareaForm" action="/subarea/saveOrUpdateSubarea.action">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>选择区域</td>
						<td>  <!-- 这个会自动帮我们进行映射的进去我们的subarea对象当中的region对象id当中 -->
							<input class="easyui-combobox" name="region.id"  
    							data-options="valueField:'id',textField:'name',url:'/subarea/getRegions.action',mode:'remote'" />  
						</td>
					</tr>
					<tr>
						<td>关键字</td>
	...
	<div class="easyui-window" title="分区添加修改" id="editSubareaWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="editSubareaForm" action="/subarea/saveOrUpdateSubarea.action">
				...
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>选择区域</td>
						<td>  <!-- 这个会自动帮我们进行映射的进去我们的subarea对象当中的region对象id当中 -->
							<input class="easyui-combobox" id="myregion" name="region.id"  
    							data-options="valueField:'id',textField:'name',url:'/subarea/getRegions.action',mode:'remote'" />  
						</td>
					</tr>
					<tr>
